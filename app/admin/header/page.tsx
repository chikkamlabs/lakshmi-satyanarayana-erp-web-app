...
export default function AdminHeader(props: any) {
  ...
  return (
    ...
    >
      ...
      <div id="admin-header-left" className="flex items-center gap-2 sm:gap-3">
        {onMenuClick && (
          <button
      ...
      {/* Middle: Greeting */}
      <div id="admin-header-middle" className="flex items-center justify-center">
        <span
          id="admin-header-greeting"
          className="font-medium text-xs sm:text-base text-[var(--primary)] px-2.5 sm:px-3 py-1 rounded-md bg-[var(--primary-light)] border border-[var(--border)] whitespace-nowrap"
        >
          Hi admin
        </span>
      </div>
      ...
      {/* Right: Logout */}
      <div id="admin-header-right" className="flex items-center">
        <button
        ...
        >
          <LogOut id="admin-logout-icon" className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[var(--danger)]" />
          <span>Logout</span>
        </button>
      </div>
    </header>
